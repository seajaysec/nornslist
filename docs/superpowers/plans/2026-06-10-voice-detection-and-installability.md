...
Run: `~/.virtualenvs/nornslist-vhmg/bin/python tasks/test_curated_installable.py`
Expected: PASS (warnings about `no-facet` rows are acceptable on stale pre-scrape data). If any HARD failure appears, the classifier is too aggressive — fix `derive_installable` (do NOT special-case names; fix the rule) and re-run.
